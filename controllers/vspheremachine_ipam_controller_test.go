...
var _ = Describe("VSphereMachine IPAM controller", func() {
	...
	AfterEach(func() {
		vSphereMachine := &v1alpha4.VSphereMachine{}
		...
		if err == nil || vSphereMachine.Name != "" {
			...
			Expect(k8sClient.Delete(context.Background(), &v1alpha4.VSphereMachine{ObjectMeta: getMeta("")})).To(Succeed())
			Eventually(func() bool {
				err := k8sClient.Get(context.Background(), NamespacedName, &v1alpha4.VSphereMachine{})
				return err != nil
			}, timeout, interval).Should(BeTrue())
		}
		Expect(k8sClient.Delete(context.Background(), &capiv1alpha4.Machine{ObjectMeta: v1.ObjectMeta{Name: MachineName, Namespace: Namespace}})).To(Succeed())
		Eventually(func() bool {
			err := k8sClient.Get(context.Background(), NamespacedName, &capiv1alpha4.Machine{})
			return err != nil
		}, timeout, interval).Should(BeTrue())
	})
	...
	Context("it finds a machine with ipam annotations", func() {
		It("handles its full lifecycle", func() {
			...
			vSphereMachine := &v1alpha4.VSphereMachine{
				...
				Spec: v1alpha4.VSphereMachineSpec{
					VirtualMachineCloneSpec: v1alpha4.VirtualMachineCloneSpec{
						Network:  v1alpha4.NetworkSpec{Devices: []v1alpha4.NetworkDeviceSpec{{NetworkName: NetworkName}}},
						Template: Template,
					},
				},
			}
			...
			createdMachine := &v1alpha4.VSphereMachine{}
			// wait for creation
			Eventually(func() bool {
		...
		It("doesn't assign an IP address when DHCP4 is enabled", func() {
			...
			vSphereMachine := &v1alpha4.VSphereMachine{
				...
				Spec: v1alpha4.VSphereMachineSpec{
					VirtualMachineCloneSpec: v1alpha4.VirtualMachineCloneSpec{
						Network:  v1alpha4.NetworkSpec{Devices: []v1alpha4.NetworkDeviceSpec{{DHCP4: true}}},
						Template: Template,
					},
				},
			}
		...
		It("fetches the annotations of the owning Machine if it doesn't have them", func() {
			...
			vmachine := &v1alpha4.VSphereMachine{
				ObjectMeta: v1.ObjectMeta{
					Name:      MachineName,
					Namespace: Namespace,
					Labels: map[string]string{
...
func checkNetworkDevices(devices []v1alpha4.NetworkDeviceSpec) bool {
	if len(devices) < 1 {
		return false
	}
	dev := devices[0]
